...
- **Proactive intent-check briefing (v0.2):** after a *significant* change (or a run of small ones), bring the user a briefing framed as an intent check — *"does this match what you had in mind?"* Delivery follows `.vibe-scribe.json` → `briefingOnChange` (`off`/`offer`/`auto`): **offer** it as an interactive yes/no selection where the host supports one (default on Claude Code); **auto-deliver a short recap** (the intent-check question + 2-3 lines, full briefing on request) where it doesn't (default on Cursor/Codex, whose plain offer is easy to miss). Earned only — genuinely significant changes; don't repeat after a decline.
...
- **Audience register (`.vibe-scribe.json` → `audience`, v0.2):** `beginner`/`intermediate`/`advanced` governs the *register* of briefings — vocabulary and detail — translated read-time over the **same canonical docs** (never forks them). Plain language + foregrounded handles for beginners; precise terms + real anchors, less hand-holding, for advanced. Register, not tone.
...
**Backstop (Claude Code + Cursor — both wired & tested): a quiet safety net.** A stop hook checks: did this session change source files but leave the docs untouched? If so, it asks the agent to update them before stopping. Otherwise it stays silent — if the docs were already updated (the normal case), the backstop never fires. Claude Code: `scripts/write-path.js` (`Stop` → `decision:block`). Cursor: `.cursor/hooks/write-path.js` (`stop` → `followup_message`, loop-safe via `loop_count`/`loop_limit`). Codex: reuses `scripts/write-path.js` via `.codex/hooks.json` (`Stop` → `decision:block`, `${PLUGIN_ROOT}` paths). Shared git gate: `scripts/lib/changes.js`. All three back the same `AGENTS.md` instruction layer.
