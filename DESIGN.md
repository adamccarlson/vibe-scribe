...
- **Presented as a selection prompt, not buried in prose.** Where the host exposes an interactive prompt primitive, present the offer as a **yes/no selection** so it can't be glazed over. **All three hosts document such a primitive** (verified against docs, 2026-06-25): Claude Code's `AskUserQuestion`, **Codex's `ask_user_question`** (a tabbed single/multi-choice questionnaire — a normal agent tool, so likely invocable from a skill), and **Cursor's ACP `cursor/ask_question`**. *Still open:* whether our skill/instruction layer can actually **invoke** each — Codex likely (plain agent tool); **Cursor is murkier**, since `cursor/ask_question` is an ACP-level primitive and in-IDE-from-a-rule multiple-choice is still partly a community feature request — plus live-testing all three. Degrade to a clear one-line text offer wherever invocation isn't available (graceful degradation, same posture as the rest of the plugin).
