...
- **Presented as a selection prompt where the host allows it — which, today, is Claude Code only.** Invocation-path check (2026-06-25), per the docs:
  - **Claude Code — YES.** `AskUserQuestion` is an agent tool; a skill runs in the agent loop, so the assistant can invoke it. The interactive prompt is a confirmed Claude Code feature.
  - **Cursor — NO.** `cursor/ask_question` is an **ACP** extension method — ACP drives Cursor's agent from an *external* client that builds its own UI; an in-IDE rule/skill cannot trigger it.
  - **Codex — NO (yet).** `ask_user_question` is an **unimplemented open feature request** (issue #9926, PR #9904 in progress) — not in shipped Codex CLI. When it merges, the agent could call it; track and flip Codex to interactive then.
  So the interactive yes/no selection is a **Claude Code highlight**; on Cursor and Codex the proactive offer degrades to a **prominent one-line text prompt** (functional, but scroll-past-able — the very thing the selection prompt is meant to avoid). The feature still works everywhere via this graceful degradation; only the flourish is Claude-specific for now.
- **Delivery model — offer vs. auto-short, set by a knob (decided 2026-06-25).** Because a text offer is skippable on hosts without an interactive prompt, the post-change behavior is configurable: `.vibe-scribe.json` → `briefingOnChange` = `off` · `offer` · `auto`.
  - `offer` (default on **Claude Code**): present a yes/no selection.
  - `auto` (default on **Cursor / Codex**, whose offer is just skippable text): **auto-deliver a *short* recap** — the intent-check question + 2-3 lines — and offer the **full** briefing on demand. Unmissable and cheap, without the unsolicited-wall-of-text backfire of auto-delivering a *full* briefing.
  - `off`: nothing.
  Earned in every mode (only genuinely significant changes).

**Audience register knob (`audience`, decided 2026-06-25).** Asked once at setup (`beginner` / `intermediate` / `advanced`), stored in `.vibe-scribe.json`; governs how the system is *explained back* — vocabulary and detail. **Read-path only:** translation over the *same* canonical docs (which already hold both registers per Lexicon entry — memorable handle + precise anchor + plain definition), never a per-audience fork, so the single-shared-source-of-truth property holds. Beginners get plain language + foregrounded handles; advanced users get precise terms + real anchors with less hand-holding. **Register, not tone** (never talk down). *Caveat:* one global level is a v1 simplification (proficiency is domain-specific); the assistant may nudge the register if the user's questions clearly contradict the setting. This makes the paradigm's "for everyone" claim real — same shared model, translated to each reader.

The config is converging into a small, legible set: `.vibe-scribe.json` → `audience` (how it's explained), `enforcement` (the safety net), `briefingOnChange` (post-change briefing delivery).
